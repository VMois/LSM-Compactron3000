    ...
    "Different input combinations produce correct results" in {
        test(new KeyChunksComparator(busWidth = 4, n = 4)).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
            val testCases = Seq(
                ...
                (0xD, 0xC, 0xB, 0xA, "b1111", "b0000", "b0001", true, false),
                (0xD, 0xC, 0xA, 0xB, "b1111", "b0000", "b0010", true, false),
                (0xD, 0xA, 0xC, 0xB, "b1111", "b0000", "b0100", true, false),
                (0xA, 0xB, 0xD, 0xB, "b1111", "b0000", "b1000", true, false),

                (0xD, 0xB, 0xC, 0xA, "b1100", "b0000", "b0100", true, false),
                (0xD, 0xB, 0xA, 0xC, "b0111", "b0000", "b0010", true, false),

                // single winner, some chunks are available
                (0xA, 0xA, 0xA, 0xA, "b1111", "b0100", "b0100", true, false),
                (0xA, 0xA, 0xA, 0xA, "b1011", "b0001", "b0001", true, false),

                // multiple winners, happens only when there are no more chunks available
                (0xB, 0xA, 0xA, 0xA, "b1111", "b1111", "b0111", true, false),
                (0xB, 0xA, 0xA, 0xA, "b1011", "b0011", "b0011", true, false),
                (0xA, 0xA, 0xA, 0xA, "b1111", "b1111", "b1111", true, false),

                // should load new chunks
                (0xD, 0xC, 0xA, 0xA, "b1111", "b0000", "b0011", false, true),
            ...
            for ((in3, in2, in1, in0, maskIn, lastChunks, maskOut, expectedWinner, shouldLoadChunks) <- testCases) {
                ...
                dut.io.shouldLoadNewChunks.expect(shouldLoadChunks.B)
            }
        }
    }
}
